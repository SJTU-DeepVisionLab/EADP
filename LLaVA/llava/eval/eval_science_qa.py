...
if __name__ == "__main__":
    ...
    if args.visual_token_num is not None:
        import datetime
        log_file = os.path.join(args.base_dir, "eval_results.jsonl")
        record = {
            "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "visual_token_num": args.visual_token_num,
            "alpha": args.alpha,
            "beta": args.beta,
            "metrics": {"accuracy": f"{correct / total * 100:.2f}%",
                        "IMG-Accuracy": f"{multimodal_correct / multimodal_total * 100:.2f}%"
                    }
        }
