...
class LlavaMetaForCausalLM(ABC):
    ...
    def local_aggregation(self, filtered_local_sim, top_entropy_vals = None, M_temp = 0.01, theta = 1.0, k_local = 5, strategy = "entropy_tempered", enable_filtered_local_sim = True):
        """
        [Local Aggregation]
        - Entropy tempered:
        ...
        if strategy == "entropy_tempered":
            ...
            if enable_filtered_local_sim:
                # Calculate Logits: S / (M * H)
                logits = filtered_local_sim / (M_temp * H_safe)
            else:
                logits = torch.ones_like(filtered_local_sim) / (M_temp * H_safe)
        ...
        elif strategy == "entropy_aware_gate":
            # Broadcast Entropy: (B, 1, M, num_keep)
            H_expanded = top_entropy_vals.unsqueeze(1)
            mu_H = H_expanded.mean(dim=-1, keepdim=True)

            # Calculate Logits: theta * (mu_H - H)
            logits = theta * (mu_H - H_expanded)

            # Calculate Weights: Sigmoid over the 'num_keep' dimension
            weights = torch.sigmoid(logits) # (B, N, M, num_keep)
    ...
    def topk_select(self, importance):
        """
        [Selection Ablation] Directly keep the tokens with highest EADP importance.
        """
        B, N = importance.shape
        device = importance.device
        T = min(getattr(self, 'visual_token_num', 0), N)
        token_quotas = torch.full((B,), T, dtype=torch.long, device=device)
        _, select_idx = torch.topk(importance, k=T, dim=-1)
        select_idx = torch.sort(select_idx, dim=-1).values

        return select_idx, token_quotas

    def dpp_select(self, importance, image_features):
        """
        [Selection Ablation] CDPruner-style conditional DPP using EADP importance
        ...
        kernel = importance.unsqueeze(2) * similarity * importance.unsqueeze(1)

        cis = torch.zeros((T, B, N), device=device)
        ...
        for i in range(T):
            ...
            eis = (kernel[batch_indices, j] - torch.einsum(
                'tb,tbn->bn',
                cis[:i, batch_indices, j],
                cis[:i],
            )) / torch.sqrt(di2s[batch_indices, j]).unsqueeze(-1)
            cis[i, :, :] = eis
            di2s -= torch.square(eis)
            di2s[batch_indices, j] = -float('inf')

        select_idx = torch.sort(select_idx.t(), dim=-1).values
    ...
    def encode_images(self, images, texts=None, split_sizes=None):
        ...
        entropy_keep_ratio = float(os.environ.get("ENTROPY_KEEP_RATIO", "0.2"))
        entropy_stability_T = float(os.environ.get("ENTROPY_STABILITY_T", "100"))
        filtered_local_sim, top_entropy_vals = self.entrpy_filter(local_sim_all, T=entropy_stability_T, entropy_keep_ratio=entropy_keep_ratio)

        # (2. 2) Local Aggregation
        m_temp = float(os.environ.get("M_TEMP", "0.01"))
        enable_filtered_local_sim = True if int(os.environ.get("ENABLE_FILTERED_LOCAL_SIM", 1)) == 1 else False
        theta = float(os.environ.get("THETA", "1.0"))
        use_entropy_aware_gate = True if int(os.environ.get("USE_ENTROPY_AWARE_GATE", 0)) == 1 else False
        use_top_k = True if int(os.environ.get("USE_TOP_K", 0)) == 1 else False
        use_negative_entropy = True if int(os.environ.get("USE_NEGATIVE_ENTROPY", 1)) == 1 else False
        k_local = int(os.environ.get("K_LOCAL", "5"))
        if use_entropy_aware_gate:
            local_sim = self.local_aggregation(filtered_local_sim = filtered_local_sim, top_entropy_vals = top_entropy_vals, theta = theta, strategy="entropy_aware_gate")
        elif use_negative_entropy:
            local_sim = self.local_aggregation(filtered_local_sim=filtered_local_sim, top_entropy_vals=top_entropy_vals, M_temp=m_temp, strategy="negative_entropy")
        elif use_top_k:
            local_sim = self.local_aggregation(filtered_local_sim = filtered_local_sim, top_entropy_vals = top_entropy_vals, k_local = k_local, strategy="top_k")
        else:
            local_sim = self.local_aggregation(filtered_local_sim = filtered_local_sim, top_entropy_vals = top_entropy_vals, M_temp = m_temp, strategy="entropy_tempered", enable_filtered_local_sim = enable_filtered_local_sim)

        # # [Plug-in] normalize first distinctively
        # # -------------------------------------------------------------------------------------------------------------
        # global_sim = (global_sim - global_sim.min(dim=-1, keepdim=True).values) / \
        #              (global_sim.max(dim=-1, keepdim=True).values - global_sim.min(dim=-1, keepdim=True).values + 1e-6)
        # local_sim = (local_sim - local_sim.min(dim=-1, keepdim=True).values) / \
        #              (local_sim.max(dim=-1, keepdim=True).values - local_sim.min(dim=-1, keepdim=True).values + 1e-6)
        # # -------------------------------------------------------------------------------------------------------------
        ...
        # (2. 5) Spatial Smoothing (env SMOOTHING_KERNEL_SIZE: 0 = skip, else kernel size; default 3)
        smoothing_kernel_size = int(os.environ.get("SMOOTHING_KERNEL_SIZE", "3"))
        if smoothing_kernel_size > 0:
            importance = self.spatial_smoothing(importance, kernel_size=smoothing_kernel_size, sigma=1.0)

        # (2. 6) Power Transform
        importance = importance ** beta

        # # [Plug-in] Exponential Transform
        # # -------------------------------------------------------------------------------------------------------------
        # den = torch.expm1(torch.tensor(beta, device=importance.device, dtype=importance.dtype))
        # importance = torch.expm1(beta * importance) / den
        # # -------------------------------------------------------------------------------------------------------------

        # # (2. 7) Importance Clipping
        # importance = self.importance_clipping(importance, importance_keep_ratio)

        # [Visualization Hook] Cache the importance scores for visualization
        ...
        selection_mode = os.environ.get("EADP_SELECTION_MODE", "").strip().lower()
        if selection_mode == "topk":
            select_idx, token_quotas = self.topk_select(importance)
        elif selection_mode == "dpp":
            select_idx, token_quotas = self.dpp_select(importance, image_features)
        else:
        ...
        # [Visualization Hook] Cache the mask for visualization
        self.pruning_mask_cache = index_masks
