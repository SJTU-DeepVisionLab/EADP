...
#
#    [DivPrune] Same framework as llava_arch_CDPruner; only encode_images() uses
#    DivPrune's diversity-based selection (iterative max-min distance) instead of DPP.
...
class LlavaMetaForCausalLM(ABC):
    ...
    # [DivPrune] Core: iterative max-min diversity selection (single sequence)
    def _divprune_select(self, visual_vectors, num_tokens_to_keep, cosine_matrix=None):
        """
        visual_vectors: (N, C). Returns selected indices (T,) and optionally cosine_matrix for reuse.
        """
        N = visual_vectors.shape[0]
        T = min(int(num_tokens_to_keep), N)
        if T <= 0:
            T = N
        device = visual_vectors.device
        if cosine_matrix is None:
            norm_mat = visual_vectors / (visual_vectors.norm(dim=1, keepdim=True) + 1e-8)
            sim = torch.mm(norm_mat, norm_mat.t())
            cosine_matrix = 1.0 - sim
        s = torch.empty(T, dtype=torch.long, device=device)
        for i in range(T):
            if i == 0:
                m2 = cosine_matrix
            else:
                m2 = torch.index_select(cosine_matrix, 0, s[:i])
            if i == 0:
                k = min(2, m2.shape[0])
                scores = torch.topk(m2, k, dim=0, largest=False).values[-1, :]
            else:
    ...
    # [DivPrune] Same interface as CDPruner: encode_images returns (image_features, index_masks)
    def encode_images(self, images, texts=None):
        vt = self.get_model().get_vision_tower()
        out = vt(images, texts=texts)
        ...
        T = getattr(self, 'visual_token_num', 0)
        if T <= 0 or T >= N:
        ...
        select_idx = torch.empty((B, T), dtype=torch.long, device=device)
        for b in range(B):
            vec = image_features[b]
            sel, _ = self._divprune_select(vec, T)
            select_idx[b] = sel
        # index_masks = torch.zeros(B, N, dtype=torch.bool, device=device)
        # index_masks.scatter_(1, select_idx, True)

        # [MODIFIED FOR JIT] Workaround for scatter on bool tensors
        index_masks = torch.zeros(B, N, dtype=torch.float32, device=device)
        index_masks = index_masks.scatter(1, select_idx, 1.0)
